"""
This module provides normalization utilities for gaze tracking data. It standardizes eye images, gaze vectors, and head pose information by transforming them into a canonical coordinate system.
This normalization enables consistent training and evaluation of gaze estimation models, regardless of variations in head pose, camera parameters, or subject distance.

Normalization Process:
- Warps input eye images so the eye center is always at a fixed distance and orientation relative to the camera.
- Transforms gaze and head pose vectors into the normalized coordinate system using scaling and rotation matrices.
- Crops and resizes the eye region for model input.

Normalized Coordinate System:
- Origin is at the camera center.
- Z-axis points from the camera to the eye center (after normalization, always at [0, 0, newdistance]).
- X/Y axes are aligned so the head is upright and facing forward.
- Focal length and image center are fixed (newfocal, imsize/2).
- All normalized images and vectors are directly comparable.

Main Components:
- norm class: Handles normalization, image warping, and coordinate transformations.
- Utility functions: Convert between 2D/3D gaze representations, flip gaze/head pose, and perform image preprocessing.

Typical Usage:
1. Initialize the norm class with eye/camera parameters.
2. Warp the input image to the normalized view.
3. Transform gaze and head pose vectors as needed.
4. Crop the normalized eye region for model input.
"""
...
class norm:
    ...
    def __ParamsCalculate(self):
        ...
        y = np.cross(z, xaxis) # type: ignore
...
# Converts 3D gaze to yaw/pitch.
def GazeTo2d(gaze):
    yaw = np.arctan2(-gaze[0], -gaze[2])
    pitch = np.arcsin(-gaze[1])
    return np.array([yaw, pitch])

# Converts yaw/pitch to 3D gaze.
def GazeTo3d(gaze):
...
# Converts head rotation vector to yaw/pitch.
def HeadTo2d(head):
...
# Flips head rotation horizontally.
def HeadFlip(head):
    ...
    z[0] = -z[0]
    x = np.cross(y, z) # type: ignore

    _ = np.array([x,y,z]) # newrot_mat
