...
# Fixed seed for reproducibility across LOOCV runs
RANDOM_SEED = 42
...
def main(data_root, test_person):
    ...
    if not os.path.isdir(label_dir):
        print(f"Error: Label directory not found at {label_dir}", file=sys.stderr)
        sys.exit(1)

    all_persons = [f"p{i:02d}" for i in range(15)]
    if test_person not in all_persons:
        print(
            f"Error: Test person '{test_person}' is not a valid ID (p00-p14).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print("-" * 50)
    print(
        f"Successfully created '{train_label_path}' with {len(all_train_lines)} samples."
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare MPIIFaceGaze labels for training."
    )
    parser.add_argument(
        "--data_root",
        type=str,
        required=True,
        help="Path to the preprocessed MPIIFaceGaze directory.",
    )
    parser.add_argument(
        "--test_person",
        type=str,
        default="p14",
        help="Person ID to hold out for the test set.",
    )
